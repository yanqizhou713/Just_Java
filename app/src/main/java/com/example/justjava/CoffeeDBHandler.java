...
public class CoffeeDBHandler extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        String query = "CREATE TABLE " + TABLE_COFFEE + "(" + COLUMN_ID + " INTEGER PRIMARY KEY AUTOINCREMENT, " + COLUMN_CUSTOMERNAME + " TEXT, " + COLUMN_SALEAMOUNT + " INTEGER );";
        db.execSQL(query);
    }
    ...
    public void deleteOrder() {
        SQLiteDatabase db = getWritableDatabase();
        db.execSQL("DELETE FROM " + TABLE_COFFEE);
    }

    public int displayTotalSales() {
        int dbString = 0;
        SQLiteDatabase db = getWritableDatabase();
        String query = "SELECT * FROM " + TABLE_COFFEE + " WHERE 1";
        // create a Cursor object to point to DB
        Cursor c = db.rawQuery(query, null);
        c.moveToFirst();
        // Loop to read and store the information in a String
        while(!c.isAfterLast()) {
            if(c.getString(c.getColumnIndex(COLUMN_CUSTOMERNAME)) != null) {
                dbString += Integer.parseInt(c.getString(c.getColumnIndex(COLUMN_SALEAMOUNT)));
            }
            c.moveToNext();
        }
        db.close();
    ...
    public String databaseToString() {
        ...
        while(!c.isAfterLast()) {
            if(c.getString(c.getColumnIndex(COLUMN_CUSTOMERNAME)) != null) {
                dbString += c.getString(c.getColumnIndex(COLUMN_CUSTOMERNAME)) + " --> $" + c.getString(c.getColumnIndex(COLUMN_SALEAMOUNT));
                dbString += "\n";
            }
            c.moveToNext();
        }
